...
class Fonts {
    ...
    static load(options) {
        this.FONTS = new Fonts(options);
    }

    static get(postscriptName) {
        if (!this.FONTS) this.load();
    ...
    constructor(options = {}) {
        ...
        if (/true/ig.test(options["system-font"])) {
            ...
            for (let file of files) {
                try {
                    ...
                    const postscript = info.name.postscriptName;
                    this.items[postscript] = {
    ...
    get(postscriptName) {
        if (postscriptName == "Arial") postscriptName = "ArialMT";
        else if (postscriptName == "Arial,Bold") postscriptName = "Arial-BoldMT";

        let font = this.items[postscriptName];
        if (font) return font;


        console.warn("Font", postscriptName, "unknown.");

        font = this.items["ArialMT"];
        if (font) return font;

        throw new Error("Failed to get font.");
    }
}
...
let PDFFont = (function PFPFontClosure() {
    ...
    let cls = function (fontObj) {
        ...
        const { opentype, filepath, style } = getFont(this.name);
