...
let PDFFont = (function PFPFontClosure() {
    ...
    cls.prototype.processText = function (p, str, maxWidth, color, fontSize, targetData, matrix2D) {
        ...
        const { width, fontBoundingBoxAscent } = this.measureText(text);
        oneText.w = PDFUnit.toFixedFloat(width);
        oneText.h = PDFUnit.toFixedFloat(fontBoundingBoxAscent);
        oneText.y = PDFUnit.toFixedFloat(oneText.y - oneText.h);
