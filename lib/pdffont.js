...
const getFont = (postscriptName) => {
    // failed to measue glyph
    if (postscriptName == "Calibri-Bold") postscriptName = "Arial-BoldMT";
    if (postscriptName == "Calibri-Italic") postscriptName = "Arial-ItalicMT";
    if (postscriptName == "Calibri") postscriptName = "ArialMT";
...
let PDFFont = (function PFPFontClosure() {
    ...
    let cls = function (fontObj) {
        ...
        if (names.length > 1) this.font = getFont(names[1]);
        else this.font = getFont(names[0]);
    };
    ...
    cls.prototype.measureText = function (text) {
        try {
            ...
            for (let i = 0; i < glyphs.length; i++) {
                ...
                //     if (glyph.index == 16 && nextGlyph == 16) { // 16: '-'
                //         width += 150 * scale;                   // j'ajoute un espace
                //     }
                // }
            ...
            return {
                ...
                fontBoundingBoxAscent: PDFUnit.toFormX(font.ascender * scale),
                fontBoundingBoxDescent: PDFUnit.toFormX(font.descender * scale)
            };
        } catch (error) {
    ...
    cls.prototype.processText = function (p, str, maxWidth, color, fontSize, targetData, matrix2D) {
        ...
        oneText.sw = PDFUnit.toFixedFloat(spaceWidth);

        const { width, fontBoundingBoxAscent, fontBoundingBoxDescent, actualBoundingBoxAscent, actualBoundingBoxDescent } = this.measureText(text);
        oneText.w = PDFUnit.toFixedFloat(width);
        oneText.h = PDFUnit.toFixedFloat(actualBoundingBoxAscent - actualBoundingBoxDescent);
        oneText.y = PDFUnit.toFixedFloat(oneText.y - actualBoundingBoxDescent - oneText.h);
